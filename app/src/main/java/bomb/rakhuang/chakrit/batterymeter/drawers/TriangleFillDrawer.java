...
public class TriangleFillDrawer extends Drawer {
    ...
    TriangleFillDrawer(Context context, int colorBack, int colorTriBack, int colorTriFore, int colorTriCritical){
        super(context);
     ...
     * @return
     */
    private Vector2D[] createTriangleVertices(float triangleSize){
     ...
     */
    private Path createTriangle(int x, int y, float triangleSize, float height, boolean flip){
        Path path = new Path();
    ...
    private Path createInsideTriangle(int x, int y, float triangleSize, float percentFilled, boolean flip){
        ...
        if(pos.distance(center) > 0) {
            ...
            new Line(new Vector2D(0, 0), _pos, 0.001);
...
/*        String labelPost = Integer.toString(Math.round(percent *100))+"%";
    ...
    private void drawDebug(Canvas c, float x, float y) {
        c.save();
        c.translate(x, y);
        Paint pp = new Paint();
