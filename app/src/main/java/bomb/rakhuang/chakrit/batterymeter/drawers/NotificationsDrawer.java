...
import bomb.rakhuang.chakrit.batterymeter.NotificationService;
import bomb.rakhuang.chakrit.batterymeter.R;

@SuppressWarnings("ALL")
public class NotificationsDrawer extends Drawer {
    private static float lastNumNotifications = -1;
    ...
    private float map(float value, float start2, float stop2){
        return start2 + (stop2 - start2) * ((value - 0f) / (2f - 0f));
    }

    /**
     * Get the color for box number i
     */
    private int getColor(int i, float max){
        if(max <= 3){
            return colors[i];
        } else {
    ...
    private float getRectHeights(float height, float num, float weight){
        ...
        return map(Math.abs(weight), h, minHeight);
     ...
     * @return
     */
    private float[] getRectYs(float yBottom, float rectHeight, int notificationLength){
